...
// 15           17 
// 20          22           24 		
bool KQuadtree::UpdateIndexList(TNode* pNode)
{
...
HRESULT KQuadtree::CreateIndexBuffer(TNode* pNode)
{
	HRESULT hr = S_OK;
...
bool  KQuadtree::SubDivide(TNode* pNode)
{
	if (pNode->m_iDepth < 3 &&
...
void KQuadtree::Buildtree(TNode* pNode)
{
	if (SubDivide(pNode))
	{
		// 0           2            4
...
TNode* KQuadtree::FindNode(TNode* pNode, KVector2 pos)
{
	do {
...
TNode*    KQuadtree::FindPlayerNode(KVector2 pos)
{
	TNode* pFindNode = FindNode(m_pRootNode, pos);
	if (pFindNode != nullptr)
	{
...
TNode* KQuadtree::CreateNode(TNode* pParent, float x, float y, float w, float h)
{
