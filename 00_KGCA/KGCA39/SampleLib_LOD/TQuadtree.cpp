...
bool TQuadtree::UpdateIndexList(KNode* pNode)
{
...
HRESULT TQuadtree::CreateIndexBuffer(KNode* pNode)
{
	HRESULT hr = S_OK;
	D3D11_BUFFER_DESC bd;
	ZeroMemory(&bd, sizeof(D3D11_BUFFER_DESC));
...
bool TQuadtree::UpdateVertexList(KNode* pNode)
{
	int iNumCols = m_pMap->m_info.m_iNumCol;
	int iStartRow = pNode->m_CornerList[0] / iNumCols;
	int iEndRow = pNode->m_CornerList[2] / iNumCols;
...
HRESULT TQuadtree::CreateVertexBuffer(KNode* pNode)
{
	HRESULT hr = S_OK;
	D3D11_BUFFER_DESC bd;
	ZeroMemory(&bd, sizeof(D3D11_BUFFER_DESC));
...
bool	TQuadtree::Render(ID3D11DeviceContext* pContext, KVector3 vCamera)
{		
...
bool	TQuadtree::Frame()
{
	...
	//KNode* pFind = FindPlayerNode(player.pos);
	//if (pFind != nullptr)
	//{
	//	cout << pFind->m_iIndex << " ";
	//}
...
bool  TQuadtree::SubDivide(KNode* pNode)
{
...
void TQuadtree::Buildtree(KNode* pNode)
{
	if (SubDivide(pNode))
	{
		// 0           2            4
...
KNode* TQuadtree::FindNode(KNode* pNode, KVector2 pos)
{
	do {
...
bool    TQuadtree::AddObject(KVector2 pos)
{
...
KNode*    TQuadtree::FindPlayerNode(KVector2 pos)
{
	KNode* pFindNode = FindNode(m_pRooKNode, pos);
	if (pFindNode != nullptr)
	{
...
KNode* TQuadtree::CreateNode(KNode* pParent, float x, float y, float w, float h)
{
