...
bool KFbxObj::CreateModel(std::wstring vsFile, std::wstring psFile)
{
	if (CreateVertexData())
	{
...
{
	m_pFbxManager = FbxManager::Create();//생성
	FbxIOSettings* ios = FbxIOSettings::Create(m_pFbxManager, IOSROOT); // 루트는 im, ex 둘다한다.
	m_pFbxManager->SetIOSettings(ios);//세팅을 넣어준다
	m_pFbxImporter = FbxImporter::Create(m_pFbxManager, "");//임포터
	...
	CreateModel(L"FbxShader.hlsl", L"../../data/shader/DefaultShader.hlsl");//재정의한 create
	return bRet;
}

void KFbxObj::PreProcess(FbxNode* pNode)
{
	// 재귀 함수를 사용해서 pNode 정보 얻기
	int iNumChild = pNode->GetChildCount();
	for (int iNode = 0; iNode < iNumChild; iNode++)
	{
		FbxNode* pChildNode = pNode->GetChild(iNode);
		FbxNodeAttribute::EType type =
			pChildNode->GetNodeAttribute()->GetAttributeType();// 열거형 타입
		if (type == FbxNodeAttribute::eMesh)//매쉬타입이면 해석
		{
...
FbxVector2 KFbxObj::ReadTextureCoord(FbxMesh* pFbxMesh, DWORD dwVertexTextureCount,
