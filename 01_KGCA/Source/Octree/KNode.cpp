...
bool KNode::AddObject(KVector3 pos)
{
...
{
	m_ObjectList.push_back(KVector3(fX, fY, fZ));
	return true;
}
bool KNode::isRect(KVector3 pos)
{
...
KNode::KNode(float x, float y, float z, float w, float h)
{
	m_pParent = nullptr;
	m_iDepth = 0;
	this->m_KRect.wh = KVector3(w, h, w);
	this->m_KRect.p0 = KVector3(x, y, z);
	//박스를 만들기 위해서 피좌표는 z 축은 정육면체니까 w와 동일
	this->m_KRect.p1 = this->m_KRect.p0 + KVector3(w, h, w);
}
