...
template<class Type>
int KTemplateLinkedList<Type>::GetAmount()
{
	int temp = 0;
	for (KNode<Type>* pNode = m_pHead; pNode->m_pNext != m_pTail;
		pNode = pNode->m_pNext)
	{
		temp++;
	}
...
{
	std::cout << "이름	국어	수학	영어	총점	평균" << std::endl;
	for (KNode<Type>* pNode = m_pHead->m_pNext;
		pNode != m_pTail;
		pNode = pNode->m_pNext)
	{
		std::cout << pNode->m_pValue->m_Name << "\t";
		std::cout << pNode->m_pValue->m_KorScore << "\t";
		std::cout << pNode->m_pValue->m_MathScore << "\t";
		std::cout << pNode->m_pValue->m_EngScore<< "\t";
		std::cout << pNode->m_pValue->m_Hap << "\t";
		std::cout << pNode->m_pValue->m_Avg<< "\n";
	}
}
