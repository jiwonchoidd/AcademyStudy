	...
	const TCHAR* pPShaderName)
{
	...
	// (1)정점버퍼 생성
	m_VB.Create(pd3dDevice, D3D11_USAGE_DEFAULT,D3D11_BIND_VERTEX_BUFFER, sizeof(PCT_VERTEX),	m_VertexList.size(),	&m_VertexList.at(0));
	m_IB.Create(pd3dDevice, D3D11_USAGE_DEFAULT,D3D11_BIND_INDEX_BUFFER, sizeof(DWORD),	m_IndexList.size(),	&m_IndexList.at(0));
	m_CB.Create(pd3dDevice, D3D11_USAGE_DYNAMIC,D3D11_BIND_CONSTANT_BUFFER, sizeof(CB_VS_PER_OBJECT), 1,NULL);
	...
	int iNumElement = sizeof(ied) / sizeof(ied[0]);
	hr = pd3dDevice->CreateInputLayout(ied, iNumElement,
		m_pVS->m_pBlob->GetBufferPointer(),	m_pVS->m_pBlob->GetBufferSize(),&m_pIL);
	if (FAILED(hr))
	{
		return false;
	}
...
bool TLineShape::Draw( D3DXVECTOR3 vS, D3DXVECTOR3 vE, D3DXVECTOR4 vColor)
{
	HRESULT hr;
	m_pImmediateContext->IASetPrimitiveTopology(
		D3D_PRIMITIVE_TOPOLOGY_LINELIST);
	...
	m_pImmediateContext->UpdateSubresource(m_VB.m_pBuffer,
...
bool TLineAxisShape::Create(ID3D11Device* pd3dDevice,
	ID3D11DeviceContext*	pImmediateContext,
	const TCHAR* pTextureName,
	const TCHAR* pVShaderName,
	const TCHAR* pPShaderName)
