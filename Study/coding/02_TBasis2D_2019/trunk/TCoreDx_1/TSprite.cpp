	...
	const TCHAR* pPShaderName)
{
	...
	m_VB.Create(pd3dDevice, D3D11_USAGE_DEFAULT,
		D3D11_BIND_VERTEX_BUFFER, sizeof(PCT), 4,
		m_VertexList);
	// (1)인덱스 버퍼 생성
	m_IndexList[0] = 0;
	m_IndexList[1] = 1;
...
bool    TSprite::Frame()
{
	...
	if (m_iTexList.size() > 0)
	{
		...
		if (m_fCurrentTime >= m_fStepTime)
		{
			...
			m_ConstantList.x = rand() / (float)RAND_MAX;
			m_ConstantList.y = rand() / (float)RAND_MAX;
			m_ConstantList.z = rand() / (float)RAND_MAX;
			m_ConstantList.w = 1.0f;
			m_ConstantList.time = g_fAccumulation;
