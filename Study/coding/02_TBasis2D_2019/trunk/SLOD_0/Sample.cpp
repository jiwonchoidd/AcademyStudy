...
bool	Sample::Init()
{
	...
	TMapDesc desc =
	{
		...
		2.0f, L"../../../data/castle.jpg",
		L"mapShader.hlsl",
	};
	m_Quadtree.Start(GetDevice(), 9, 9);
	...
	m_Quadtree.InitLOD(m_Quadtree.m_pRootNode, g_iLodLevel);
	if (m_Quadtree.BuildTree(m_Quadtree.m_pRootNode))
	{
		for (int iNode = 0; iNode <	4; iNode++)
		{
			m_Quadtree.SetNeighborNode(m_Quadtree.m_pRootNode->m_pChild[iNode]);
		}
		m_Quadtree.SetLOD(m_Quadtree.m_Desc.Width, m_Quadtree.m_dwNumDepth);
	}

	for (int iBox = 0; iBox < 100; iBox++)
	{
		m_MapObj[iBox].Set(&m_BoxObj, m_Quadtree.m_Desc.Width*desc.size,
			m_Quadtree.m_Desc.Height*desc.size);
		m_Quadtree.AddObject(m_Quadtree.m_pRootNode, &m_MapObj[iBox]);
		//m_Quadtree.AddObject(&m_MapObj[iBox]);
		m_MapObj[iBox].m_matWorld._42 = m_Quadtree.GetHeight(
			m_MapObj[iBox].m_matWorld._41,
			m_MapObj[iBox].m_matWorld._43);				
	}
	// 상수 버퍼
	...
	D3DXVECTOR3 vCameraPos = D3DXVECTOR3(0, 10, -10.0f);
	D3DXVECTOR3 vTargetPos = vCameraPos + D3DXVECTOR3(0, 0, 1) * 10.0f;
	vCameraPos.y = m_Quadtree.GetHeight(vCameraPos.x,vCameraPos.z) + 50.0f;
	m_pMainCamera->SetViewMatrix(vCameraPos,vTargetPos);
...
bool	Sample::Render()
{
	...
	m_Quadtree.SetMatrix(m_matWorldMap,m_pMainCamera->m_matView,m_pMainCamera->m_matProj);
	m_WaterCB.Apply(GetContext(), 1, 0);
	m_Quadtree.Render();	

	static int iDrawObject = 0;
	for (int iNode = 0; iNode < m_Quadtree.m_RenderList.size(); iNode++)
	{
		iDrawObject += m_Quadtree.DrawChild(m_Quadtree.m_RenderList[iNode]);
	}
	TCHAR pbuffer[256] = { 0, };
	_stprintf_s(pbuffer, L"\n ->>Num Object=%d", iDrawObject);//	m_Quadtree.m_iNumFace);
...
{
	/*m_pMainCamera->m_vEye.y = m_Quadtree.GetHeight(
		m_pMainCamera->m_vEye.x,
		m_pMainCamera->m_vEye.z) + 50.0f;*/
	return true;
}
bool    Sample::Frame()
{
	...
	for (int iBox = 0; iBox < 100; iBox++)
	{
		m_MapObj[iBox].m_bDraw = false;
		m_MapObj[iBox].m_bCollision = false;
	}
	// 지형 랜더링
	m_Quadtree.Frame();
