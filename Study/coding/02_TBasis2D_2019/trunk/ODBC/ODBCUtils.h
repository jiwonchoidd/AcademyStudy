...
char getInt(const char *label, int *retInt, char confirm, int quitVal);
void extract_error(SQLCHAR *fn, SQLHANDLE handle, SQLSMALLINT type);
...
char getStr(const char *label, char *retStr, int len, char confirm) {
...
int getDataSources(SQLHANDLE env, int maxNo, int maxLen, char dsns[MAXDSNS][MAXDSNLEN])
{	
	SQLRETURN retcode;
	SQLTCHAR dsnName[256];
	SQLSMALLINT dsnNameLenReturned;
	SQLTCHAR driverDesc[256];
		...
		&driverDescLenReturned))) 
	{
		...
		strcpy(&dsns[count][0], (char*)dsnName);
		count++;
	}
...
//
void dumpDescriptorRec(const char *comment,
	SQLHDESC descriptor,
	int incBookmarkRec) {
...
void dumpDescriptorHeaderFields(SQLHDESC descriptor) {
	...
	SQLINTEGER         descArraySize = 0;
	SQLUSMALLINT   *descArrayStatusPtr;
	SQLINTEGER         *descBindOffsetPtr;
	SQLINTEGER      descBindType = 0;
	SQLSMALLINT     descCount = 0;
	SQLINTEGER        *descRowsProcessesPtr;
...
void dumpDescriptors(SQLCHAR *comment,         // display heading/comment
	...
	CHECK_ERROR(retcode, "SQLGetStmtAttr(SQL_HANDLE_STMT)",hstmt, SQL_HANDLE_STMT);
