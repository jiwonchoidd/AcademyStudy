...
{
	//float x, y, z, w;
	float index; // 컬러 // 레지스터 단위 (float*4)	
	float a, b, c;
};
struct TObject
{
	...
	ID3D11ShaderResourceView* m_pTexBlendA;
	ID3D11ShaderResourceView* m_pTexBlendB;
...
public:
	...
	ID3D11BlendState*	  g_pNoAlphaBlend;
	ID3D11BlendState*	  g_pColorBlend;
	ID3D11BlendState*	  g_pFactorBlend;
	TObject				m_Object[MAX_OBJECT];
	ID3D11ShaderResourceView* m_pTexArraySRV[MAX_TEXTURE];
	...
	ID3D11Buffer*		m_pIB;
	ID3D11Buffer*		m_pCB;//ConstantBuffer
	ID3D11Buffer*		m_pCBPixel;//ConstantBuffer
