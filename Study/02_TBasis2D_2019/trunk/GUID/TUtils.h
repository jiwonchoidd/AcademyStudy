...
inline void SAFE_FREESID(PSID& x)
{
	if (x != NULL) {
		FreeSid(x);
		x = NULL;
...
inline void SAFE_CLOSE(	HANDLE& x)
{
...
inline void THROW_ON_FAILED(const char* /*strMessage*/,	HRESULT hr)
{
	if (FAILED(hr)) {
		throw hr;
	}
...
LPWSTR NewString(LPCWSTR pwszSource);

LPWSTR NewString(std::wstring& wsSrc);

std::string GuidToString(GUID& guid);

std::wstring GuidToWString(	GUID& guid);

GUID WStringToGuid(	std::wstring& value);

HRESULT UnicodeToAnsi(	__in LPCWSTR pwszIn,	__out LPSTR&  pszOut);

HRESULT AnsiToUnicode(	__in LPCSTR pszIn,	__out LPWSTR& pwszOut);

HRESULT AnsiToGuid(	LPCSTR szString,	GUID& guid);

LPSTR GuidToAnsi(GUID& guid);

HRESULT GetEnvVar(std::wstring& var,std::wstring& value);

//
// Tracing
//

void TraceMsg(	LPCWSTR msg,...);
