...
bool TWrite::Reset(IDXGISurface1*  pSurface)
{
	...
	hr = m_pD2DF->CreateDxgiSurfaceRenderTarget(
		pSurface,
...
bool		TWrite::Set(HWND hWnd, int iWidth, int iHeight, IDXGISurface1*  pSurface)
{
	...
	m_pD2DRT->CreateSolidColorBrush(
		D2D1::ColorF(D2D1::ColorF::Red), &m_pBrush);
	return true;
}
bool		TWrite::Init() {
	return true;
...
bool		TWrite::Draw(D2D_RECT_F& rc, TCHAR* pText,
	D2D1::ColorF color)
{
	if (m_pD2DRT == NULL) return false;
	m_pBrush->SetColor(color);
	m_pD2DRT->DrawTextW(pText, wcslen(pText),
		m_pWTF, rc, m_pBrush);
	return true;
}
bool		TWrite::Render()
