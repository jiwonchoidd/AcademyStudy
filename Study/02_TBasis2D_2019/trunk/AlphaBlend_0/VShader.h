...
cbuffer cb0
{
	...
	float  index : packoffset(c1.y);    // c1
};
...
VS_OUTPUT VS3(VS_INPUT input)
{
	...
	output.p.z = pos.z;
	output.p.w = 1.0f;
	output.t = input.t;// +float2(Color.x, Color.y);// input.t;
	output.c = input.c;// float4(Color.x, Color.y, Color.z, cos(time)*0.5f + 0.5f);
	output.c.a = cos(time)*0.5f + 0.5f;
	return output;
}